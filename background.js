...
function statusDisplay(){
  ...
  if(msg){
     document.getElementsByClassName("faq-link")[0].innerHTML=msg;
  }
}
...
function start(districtId){
  ...
  setTimeout(function(){
    //alert("Appointment Booked for "+ BENEFICIARIES_NAMES + " at "+ BOOK_LOCATION_NAME +". Please check the details at cowin.gov.in.");
  },20000);
}
function getDistrict(){
      ...
      beneficiariesFirstCheckLogin = true;
function checkCoWinAvailability()
               ...
               for(var i in data["centers"]){
                      ...
                      for(var j in centerData["sessions"]){
                          ...
                          if(age == 18 && avail > 5){
                              ...
                              BOOK_LOCATION_NAME = name;
                              bookAppointment(centerId,sessionId,slot);
                              setTimeout(function()
                                {
                                  alert(avail + "  slots available at "+name+ " on " + date);
                                },  10);
                                break;
      ...
      var token = sessionStorage["userToken"] ? sessionStorage["userToken"].replaceAll('"','') : "";
      xmlhttp.setRequestHeader("authorization", "Bearer "+ token );
      xmlhttp.send();
      };

      firstTimeBeneficiaries = true;
            ...
            if (xmlhttp.readyState==4)
            {
              if(xmlhttp.status==200)
              {
                ...
                for(var i in data["beneficiaries"]){
                    ...
                    if(bData["vaccination_status"] == "Not Vaccinated" && bData["dose1_date"]==""){
                      ...
                      names.push(bData["name"]);
                    }
                    BENEFICIARIES = beneficiaries;
                }
                if(firstTimeBeneficiaries){
                  alert(names.join(", ") + " added as beneficiaries.");
                  BENEFICIARIES_NAMES=names.join(", ");
                  firstTimeBeneficiaries = false;
                }
              ...
              }else if(xmlhttp.status==401)
              {
                ...
                if(beneficiariesFirstCheck){
                  ...
                  beneficiariesFirstCheck= false;
                }else if(beneficiariesFirstCheckLogin && beneficiariesFirstCheck){
                  alert("You are not logged in. You will still get the availability alert. To autobook please login.");
                  beneficiariesFirstCheckLogin = false;
                }	
              }
            }
          }
      ...
      function bookAppointment(centerId,sessionId,slot){
          ...
          setTimeout(function() {
                ...
                alert("Missed booking for availability at "+BOOK_LOCATION_NAME+". Now looking for other slots available.");
              }
          }, 5000);
          xmlhttp.onreadystatechange=function()
          {
            ...
            if (xmlhttp.readyState==4){
              if(xmlhttp.status==200)
              {
                ...
                console.log("Appointment Booked for "+ BENEFICIARIES_NAMES + " at "+ BOOK_LOCATION_NAME +". Please check the details at cowin.gov.in.");
                alert("Appointment Booked for "+ BENEFICIARIES_NAMES + " at "+ BOOK_LOCATION_NAME +". Please check the details at cowin.gov.in.");
              }
